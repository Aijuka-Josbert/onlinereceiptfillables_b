    ...
    function convertChunk(chunkNum) {
        ...
        if (rest > 0) {
            if (rest < 10) words += units[rest] + ' ';
                ...
                const ten = Math.floor(rest / 10), unit = rest % 10;
                words += tens[ten] + ' ';
...
function applyTheme(doc) {
    ...
    document.querySelectorAll(`.swatch-row[data-doc="${doc}"] .swatch`).forEach(sw => {
        const idx = parseInt(sw.dataset.paletteIndex, 10);
        const p = PALETTES[idx];
        const matches = p.paper.toLowerCase() === t.paper.toLowerCase() &&
...
function buildSwatches() {
    ...
    document.querySelectorAll('.color-pick input[type="color"]').forEach(inp => {
        const [doc, , key] = inp.id.split('-'); // e.g. "dn-c-paper" -> dn, c, paper
        const propMap = { paper: 'paper', ink: 'ink', accent: 'accent' };
        inp.addEventListener('input', () => setThemeColor(doc, propMap[key], inp.value));
    });
...
function renderRows(docKey) {
    ...
    container.querySelectorAll('input').forEach(inp => inp.addEventListener('input', onRowInput));
    container.querySelectorAll('.remove-row').forEach(btn => btn.addEventListener('click', onRemoveRow));
}
...
function saveToStorage() {
    ...
    } catch (e) { /* storage may be full or unavailable — ignore */ }
}
...
$('btn-today').addEventListener('click', () => { setTodayDates(); renderPreview(); saveToStorage(); });
...
function clearAllData() {
    if (confirm('Delete all saved data (including logo and colors) and reset to defaults?')) {
